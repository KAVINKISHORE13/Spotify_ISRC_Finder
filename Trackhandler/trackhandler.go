...
// @Success 200 {object} model.TrackDeatils	"Track details"
...
var requestBody model.RequestBody
		...
		if err != nil {
			if strings.Contains(err.Error(), "track already exist") {
				c.JSON(500, gin.H{"error": "track already exists"})
				return
			}
			c.JSON(500, gin.H{"error": "Failed to create track"})
			
			return
		}

		c.JSON(200, gin.H{"message": "Track created successfully", "track": track})
}
...
// @Success 200 {object} model.TrackDeatils "Successfully retrieved tracks"
...
// @Param existingTrack body model.TrackDeatils true "Updated track details"
// @Success 200 {object} model.TrackDeatils "Track updated successfully"
